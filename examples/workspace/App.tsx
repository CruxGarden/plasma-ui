...
interface Mail { id: number; from: string; subject: string; body: string }
const MAIL: Mail[] = [
  { id: 1, from: "Ana", subject: "Launch checklist", body: "Docs page is live. npm publish is the last step - want to pair at 4?" },
...
const TASKS = ["Tag v0.1.0", "Write announcement", "Record demo clip", "Publish to npm"];

interface Layout { [id: string]: Offset }
...
export function App() {
  ...
  const [done, setDone] = useState<boolean[]>(() => TASKS.map((_, i) => i === 0));
  ...
  const home: Layout = {
    inbox: { x: 0, y: 0 }, reader: { x: col, y: 0 }, tasks: { x: 0, y: row }, player: { x: col, y: row },
  };
  const size = {
  ...
  useEffect(() => { if (Object.keys(layout).length) try { localStorage.setItem(KEY, JSON.stringify(layout)); } catch {} }, [layout]);
          ...
          <div className="plate row">
            ...
            <div className="seg" role="group" aria-label="Mood">
              {(["tidal", "aurora", "ember"] as MoodName[]).map(m => (
                <button key={m} aria-pressed={mood === m} onClick={() => setMood(m)}>{m[0].toUpperCase() + m.slice(1)}</button>
              ))}
            </div>
            <label className="frost">Frost
              <input type="range" min={0} max={1} step={0.05} value={frost} onChange={e => setFrost(+e.target.value)} data-plasma-nodrag />
            </label>
            <button className="ghost" data-plasma-nodrag onClick={() => setLayout({ ...home })}>Reset layout</button>
          </div>
        </Plasma>

        <div className="body">
          <Plasma as="nav" className="dock" radius={22} lean={false} padding={18} fuse={false} aria-label="Dock">
            <div className="plate col">
              {["📥", "✓", "▶", "⚙"].map(g => <button key={g} className="dockbtn" aria-label="Dock item">{g}</button>)}
            </div>
          </Plasma>

          <div ref={stage} className="stage">
            <Plasma className="panel inbox" {...place("inbox")} aria-label="Inbox panel">
              <div className="plate col">
                <h2>Inbox</h2>
                <ul data-plasma-nodrag>
                  {MAIL.map(m => (
                    <li key={m.id}>
                      <button aria-pressed={m.id === mailId} onClick={() => { setMailId(m.id); setFocused("reader"); }}>
            ...
            <Plasma className="panel reader" {...place("reader")} aria-label="Reader panel">
              <div className="plate col">
                <h2>{mail.subject}</h2>
            ...
            <Plasma className="panel tasks" {...place("tasks")} aria-label="Tasks panel">
              <div className="plate col">
                <h2>Tasks</h2>
                <ul data-plasma-nodrag>
                  {TASKS.map((t, i) => (
            ...
            <Plasma className="panel player" {...place("player")} aria-label="Player panel">
              <PlayerContent />
            </Plasma>
          </div>
        </div>
...
function PlayerContent() {
  ...
  return (
    <div className="plate col">
      ...
      <div className="progress"><span style={{ width: `${(t / 224) * 100}%` }} /></div>
      <div className="row">
        <span className="time">{mmss(t)} / 3:44</span>
        <button className="ghost" data-plasma-nodrag onClick={() => { setOn(o => !o); bump(0.6); }}>{on ? "Pause" : "Play"}</button>
      </div>
    </div>
  );
}
